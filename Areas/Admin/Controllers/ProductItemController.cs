...
namespace MemberShipWebsite.Areas.Admin.Controllers
{
    public class ProductItemController : Controller
    {
        ...
        private async Task<ProductItem> GetProductItem(int? itemId, int? productId)
        {
            try
            {
                ...
                var productItem = await db.ProductItems.FirstOrDefaultAsync(mi => mi.ProductId.Equals(prodId) && mi.ItemId.Equals(itmid));
                return productItem;
            }
            catch (Exception)
            {
    ...
    public async Task<ActionResult> Edit([Bind(Include = "ProductId,ItemId,OldProductId,OldItemId")] ProductItem productItem)
    {
        if (ModelState.IsValid)
        {
             bool canChange = await productItem.CanChange(db);
                if (canChange)
